#!/usr/bin/env python3
"""Embucket-only two-schema parity check.

Counterpart to parity_self.py but runs against Embucket via the
Lambda client. Same methodology: rowcount + MD5 row-hash on the three
Snowplow golden tables, keyed on natural keys, with model_tstamp and
event_id excluded from the hash.

Usage:
    uv run python scripts/parity_self_embucket.py \\
        --left  atomic_derived_baseline_fr \\
        --right atomic_derived_patched_fr
"""
...
# Columns mirror parity_self.py (Snowflake variant): model_tstamp and
# event_id are excluded (non-deterministic between pipeline variants).
TABLES = [
    TableSpec(
...
def fetch_rowcount(client, token, fqn: str) -> int:
    body = embucket_client.run_sql(client, LAMBDA_ARN, token,
                                   f"SELECT COUNT(*) FROM {fqn}")
    return int(body["data"]["rowset"][0][0])


def fetch_hashes(client, token, fqn: str, spec: TableSpec) -> dict:
    cols = ", ".join(spec.columns)
    body = embucket_client.run_sql(client, LAMBDA_ARN, token,
                                   f"SELECT {spec.natural_key}, {cols} FROM {fqn}")
    return {row[0]: row_hash(list(row[1:])) for row in body["data"]["rowset"]}
...
def print_diff(spec: TableSpec, diff: DiffResult) -> None:
    ...
    print(f"  only_left:   {len(diff.only_left)}")
    print(f"  only_right:  {len(diff.only_right)}")
...
def run(left_schema: str, right_schema: str) -> int:
    ...
    for spec in TABLES:
        left_fqn = f"demo.{left_schema}.{spec.name}"
        right_fqn = f"demo.{right_schema}.{spec.name}"
        lc = fetch_rowcount(client, token, left_fqn)
        rc = fetch_rowcount(client, token, right_fqn)
        ...
        if lc == 0:
            ...
            continue
        lh = fetch_hashes(client, token, left_fqn, spec)
        rh = fetch_hashes(client, token, right_fqn, spec)

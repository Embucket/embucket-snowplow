...
{% macro default__base_create_snowplow_events_this_run(sessions_this_run_table, session_identifiers, session_sql, session_timestamp, derived_tstamp_partitioned, days_late_allowed, max_session_days, app_ids, snowplow_events_database, snowplow_events_schema, snowplow_events_table, entities_or_sdes, custom_sql) %}
    ...
    {# event_id dedup removed: this macro reads from the raw source events table (all time).
       A CTE-based narrow-projection dedup would double-scan the full table (DataFusion
       inlines CTEs), exceeding the Lambda memory budget. The original QUALIFY also OOMs
       because it buffers 137+ columns. Dedup is handled by downstream models which
       each dedup by their own key (page_view_id, domain_sessionid, domain_userid)
       on the much smaller batch-sized base_events_this_run table. #}
